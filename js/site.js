...
const publicKey = 'BPYz6QaFAdmZOvaXnToQFvsZv0Mw7R7vwtTDRf1WDZ6ovFjuy-g--berCODcg-owTTDjyjLjDeSWrfDvYRNhbKY';

navigator.serviceWorker && navigator.serviceWorker.register('sw.js').then(function(registration) {
  console.log('Excelente! Registrado com o escopo: ', registration.scope);
});
...
navigator.serviceWorker && navigator.serviceWorker.ready.then(function(serviceWorkerRegistration) {  
  serviceWorkerRegistration.pushManager.getSubscription()  
    .then(function(subscription) {  
      ...
      if (subscription) {
        console.info('Já existe!', subscription);
      ...
      serviceWorkerRegistration.pushManager.subscribe({
        ...
        .then(function(subscription) { 
          console.info('Feito! Push Menssagem inscrito!', subscription);
          window.subscription = subscription;
        });
    });
});
...
self.addEventListener('push', function(event) {
  event.waitUntil(
    self.registration.showNotification('Teste de Push', {
      body: 'Push Message recebido!'
   }));
});
